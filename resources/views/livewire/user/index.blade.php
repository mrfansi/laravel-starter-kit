            ...
            <flux:table :rows="$users">
                ...
                <flux:table.body>
                    <template x-for="row in $wire.users.data" :key="row.id">
                        <flux:table.row x-bind:wire-key="'user-' + row.id">
                            ...
                            <flux:table.cell class="text-right space-x-2">
                                <flux:button variant="outline" size="sm" x-bind:href="`/users/${row.id}/edit`"
                                    wire:navigate>
                                    {{ __('Edit') }}
                                </flux:button>
                                <flux:dialog>
                                    <flux:dialog.trigger>
                                        <flux:button variant="destructive" size="sm"
                                            x-on:click="$wire.confirmDelete(row.id)" wire:loading.attr="disabled">
                                            {{ __('Delete') }}
                                        </flux:button>
                                    </flux:dialog.trigger>
                                    <flux:dialog.content>
